...
```{r people-bias, echo = FALSE, fig.cap='Remember that people have a lot of internal biases and will appear differently than what they really are. Photo by Juan Pablo Serrano Arenas from Pexels'}
...
```{r baby-crying, echo = FALSE, fig.cap='Crying babies were patterns that data would never reveal to me. Photo by Ba Phi from Pexels.'}
...
In the last chapter we discussed that one of the simplest ways for creating valuable information with Data Science is by describing a population which is of our interest. Our toolset is quite limited though so far - we can only use *measures of central tendency*. It is now time to revisit this idea and expand on it with distributions, visualisations and *measures of spread*.

### Distributions

*A picture is worth a thousand words*, is an old proverb which is true also for us. Within Data Science, this topic is called Data Visualisation and is very broad. We only tip our toes into its waters now by learning how can we **visualise a distribution**. 

Let's revisit an example from a previous chapter, where we had salaries distributed such as this:
...
We hav ein the last chapter learned about mode, median and mean. Out of these, median and mean will be very useful for you when you will be thinking essentally about any population. We expand our knowledge now on *median* with a knowledge that this is so called **50th per
